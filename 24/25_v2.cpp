...
// Helper function to get next node in clockwise traversal of rectangular structure
// Takes current node, current edge (0=top,1=right,2=bottom,3=left), nodes passed in edge, and limits
CNode* getNextClockwise(CNode* curr, int& edge, int& nodesInEdge, int h, int w) {
    int edgeLimits[4] = {h, w-1, h-1, w-2};  // Nodes per edge
    
    // Check if we need to move to next edge
    if(nodesInEdge >= edgeLimits[edge]) {
        edge = (edge + 1) % 4;   // Move to next edge
        nodesInEdge = 0;         // Reset counter
    }
    
    CNode* next = NULL;
    // Move based on current edge direction
    if(edge == 0) next = curr->pUp;         // Top edge: go UP
    else if(edge == 1) next = curr->pNext;  // Right edge: go RIGHT
    else if(edge == 2) next = curr->pDown;  // Bottom edge: go DOWN
    else if(edge == 3) next = curr->pPrev;  // Left edge: go LEFT
...
int main(){
    ...
    for(int nodeIdx = 0; nodeIdx < n; nodeIdx++){
        ...
        CNode* cycleHead = NULL;    // Pointer to the first node in the cycle (for traversal)
        ...
        int edgeLimits[4];          // Number of nodes per edge
        edgeLimits[0] = h;          // Top edge: h nodes going up
        edgeLimits[1] = w - 1;      // Right edge: w-1 nodes going right
        edgeLimits[2] = h - 1;      // Bottom edge: h-1 nodes going down
        edgeLimits[3] = w - 2;      // Left edge: w-2 nodes going left
        ...
        for(int i = 0; i < totalNodes; i++){
            ...
            if(i == 0) {                          // If this is the first node in the cycle
                cycleHead = newNode;              // Set as head of cycle
                firstNode = newNode;              // Remember first node for closing cycle
            ...
            if(nodesInCurrentEdge == edgeLimits[edgeCounter]) {
        ...
        CNode* temp = cycleHead;   // Start from the head of the cycle
        ...
        for(int i = 1; i < totalNodes; i++){
            temp = getNextClockwise(temp, edge, nodesInEdge, h, w);  // Get next node
        ...
        temp = cycleHead;      // Start from beginning
        edge = 0;              // Reset edge
        ...
        // Traverse and count
        for(int i = 1; i < totalNodes; i++){
            temp = getNextClockwise(temp, edge, nodesInEdge, h, w);
